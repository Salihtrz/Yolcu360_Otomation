...
namespace Yolcu360.BusinessLayer.Abstract
{
    ...
    public interface IRentalSimulationService
    {
        ...
        /// </summary>
        Task<RentalSimulationSummaryDto> CreateRentalAsync(CreateSimulatedRentalDto dto, CancellationToken ct = default);

        /// <summary>Tüm simülasyonları (özet, ek hizmet hariç) en yeniden eskiye listeler.</summary>
        Task<List<ResultSimulatedRentalDto>> GetAllRentalAsync(CancellationToken ct = default);

        /// <summary>Tek simülasyon (ek hizmet hariç); yoksa null.</summary>
        Task<ResultSimulatedRentalDto> GetByIdRentalAsync(int id, CancellationToken ct = default);

        /// <summary>Tek simülasyon, ek hizmetleriyle birlikte (detay/özet için); yoksa null.</summary>
        Task<ResultSimulatedRentalDto> GetRentalWithExtrasAsync(int id, CancellationToken ct = default);

        /// <summary>Bir simülasyon kaydını (ve ek hizmetlerini) siler.</summary>
        Task DeleteRentalAsync(int id, CancellationToken ct = default);
    }
}
